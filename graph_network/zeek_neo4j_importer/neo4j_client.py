...
def create_constraints(session: Any, labels: dict[str, str]) -> None:
    ...
    source_idx = cypher_ident(f"{labels['source'].lower()}_import_value_unique")
    destination_idx = cypher_ident(f"{labels['destination'].lower()}_import_value_unique")
    event_idx = cypher_ident(f"{labels['event'].lower()}_event_id_unique")
    event_ts_idx = cypher_ident(f"{labels['event'].lower()}_ts_datetime")

    statements = [
        f"""
        CREATE CONSTRAINT `{source_idx}` IF NOT EXISTS
        FOR (n:`{source_label}`)
        REQUIRE (n.import_label, n.value) IS UNIQUE
        """,
        f"""
        CREATE CONSTRAINT `{destination_idx}` IF NOT EXISTS
        FOR (n:`{destination_label}`)
        REQUIRE (n.import_label, n.value) IS UNIQUE
        """,
        f"""
...
def write_batch(tx: Any, rows: list[dict[str, Any]], labels: dict[str, str], config: dict[str, Any]) -> None:
    ...
    if config["graph"]["edge_mode"] == "event":
        ...
        tx.run(
            ...
            MERGE (s:`{source_label}` {{import_label: row.import_label, value: row.src_value}})
            SET s.source_fields = row.src_fields,
                s.is_source = true,
                s.name = row.src_value,
                s.display = row.src_display,
                s.caption = row.src_display

            MERGE (d:`{destination_label}` {{import_label: row.import_label, value: row.dst_value}})
            SET d.destination_fields = row.dst_fields,
                d.is_destination = true,
                d.name = row.dst_value,
                d.display = row.dst_display,
                d.caption = row.dst_display
