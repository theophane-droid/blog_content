...
def build_row(record: dict[str, Any], config: dict[str, Any], path: Path, line_no: int) -> dict[str, Any] | None:
    ...
    src_value = composite_value(record, graph["source_fields"])
    dst_value = composite_value(record, graph["destination_fields"])
    ...
    source_caption = caption_from_values(
        ...
        record,
        display_value(graph["source_fields"], src_value),
    )

    destination_caption = caption_from_values(
        graph.get("destination_caption_fields", []),
        record,
        display_value(graph["destination_fields"], dst_value),
    )
    ...
    edge_key_parts = [
        "|".join(graph["source_fields"]),
        src_value,
        "|".join(graph["destination_fields"]),
        dst_value,
