...
export function displayFromFields(data, fields, fallback) {
  ...
  for (const field of fields) {
    ...
    if (value !== "" && value !== undefined && value !== null) parts.push(`${field}=${value}`);
  }
...
export function applyStyle() {
  ...
  state.cy.edges().forEach((edge) => {
    ...
    edge.removeClass("kerberos-edge conn-edge has-note virtual-edge event-link-edge");
  ...
  state.cy.nodes().forEach((node) => {
    node.removeClass("has-note custom-color");
  ...
  state.cy.style()
    ...
    .selector("edge.virtual-edge").style({ opacity: 0.72, "line-style": "solid", "line-color": "rgba(0,245,212,0.68)", "target-arrow-shape": "triangle", "target-arrow-color": "rgba(0,245,212,0.68)" })
    ...
    .selector("edge.has-note").style({ "line-color": "#f4a261", opacity: 0.72 })
