...
export function updateControlsFromGraph(graph) {
  const nodeFields = new Set(["label", "caption", "display", "name", "value", "__note"]);
  const edgeFields = new Set(["label", "caption", "display", "name", "type", "event_count", "total_bytes", "total_duration", "first_seen", "last_seen", "services", "destination_ports", "__note"]);
...
export async function loadSchema() {
  ...
  state.schema = schema;
  const allFields = [...new Set(["label", "caption", "display", "name", "value", "event_count", "total_bytes", "total_duration", "first_seen", "last_seen", "services", "destination_ports", "__note", ...schema.propertyKeys])].sort();
