...
export function updateSelection(element) {
  ...
  el("selectionText").textContent = element.isNode()
    ? `Node ${data.caption || data.id}${note ? " · note" : ""}`
    : `Edge ${data.type || data.caption}${note ? " · note" : ""}`;
  el("inspector").textContent = JSON.stringify({ ...data, note, customColor: color, properties: props }, null, 2);
