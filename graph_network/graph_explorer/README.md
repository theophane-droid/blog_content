# Neo4j Cytoscape Network Event Explorer
...
- Event Explorer now supports a custom Neo4j `WHERE` expression through **Neo4j WHERE filter**.
  ...
  - Example: `toInteger(e.id_resp_p) IN $ports AND e.proto = 'tcp'`
  - Parameters go in **Filter parameters JSON**, for example: `{ "ports": [80, 443] }`.
  - The field/value filter and text search still work and are combined with the custom expression.
...
- Event Explorer controls were grouped into collapsible **Filters** and **Limits** sections to reduce button noise.
